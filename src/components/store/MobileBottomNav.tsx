          ...
          return (
            ...
            >
              <div className="relative">
                ...
                {item.badge ? (
                  <span className="absolute -top-1.5 -right-2.5 bg-primary text-primary-foreground text-[8px] font-bold rounded-full min-w-[16px] h-4 flex items-center justify-center px-0.5">
                    {item.badge > 9 ? "9+" : item.badge}
                  </span>
                ) : null}
              </div>
              <span className={`text-[9px] font-medium ${isActive ? "font-bold" : ""}`}>{item.label}</span>
            </Link>
          );
        })}
      </div>
